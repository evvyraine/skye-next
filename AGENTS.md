...
- **Access is deny-by-default for groups.** Private Telegram and web chat are allowed on Free unless the user is banned. The owner is always allowed. A ban beats every allow except the immutable owner. Groups stay allowlist-only — Free and Plus do not grant group access. Allowlisting a group grants access only inside that group. Skye Plus (or complimentary/owner access) is required to create, edit, or share custom agents.
